...
function addAnnouncementsToTable()
{
	...
	for (var i = 0; i < announcementList.length; i++)
	{
		...
		tableRow.id = "announcementRow" + i.toString();
		...
		dateCreatedCell.innerHTML = announcementList[i].timestamp;
		locationCell.innerHTML = announcementList[i].location;
		deadlineCell.innerHTML = announcementList[i].deadline;
		
		switch (parseInt(new Date().getDate()) - parseInt(announcementList[i].timestamp.toString().substring(9,11)))
		{
			case 1:
				dateCreatedCell.innerHTML = "Yesterday";
				break;
			case 2:
				dateCreatedCell.innerHTML = "Day Before Yesterday";
				break;
			case 3:
				dateCreatedCell.innerHTML = "3 Days Ago";
				break;
			case 4:
				dateCreatedCell.innerHTML = "4 Days Ago";
				break;
			case 5:
				dateCreatedCell.innerHTML = "5 Days Ago";
				break;
			case 6:
				dateCreatedCell.innerHTML = "6 Days Ago";
				break;
			case 7:
				dateCreatedCell.innerHTML = "1 Week Ago";
				break;
			default:	
		}
...
function onLoad()
{
	...
	$('#logo').css('width', parseInt($('#blueHeader').height(), 10) + parseInt($('#blueHeader').css('padding-bottom'), 10));
	$('#logo').css('height', parseInt($('#blueHeader').height(), 10) + parseInt($('#blueHeader').css('padding-bottom'), 10));
	...
	if (isMobileDevice())
	{
		...
		$('#logo').css('width', parseInt($('#blueHeader').height(), 10) + parseInt($('#blueHeader').css('padding-bottom'), 10));
		$('#logo').css('height', parseInt($('#blueHeader').height(), 10) + parseInt($('#blueHeader').css('padding-bottom'), 10));
		alert(parseInt($('#blueHeader').height(), 10) + parseInt($('#blueHeader').css('padding-bottom'), 10));
...
window.onscroll = function combineHeaderWithNavbar()
{
	...
	if (!isMobileDevice())
	{
		...
		$('#main').css('padding-top', (($('#containerHeader').height() / $(document).height() * 100) + 10).toString() + "%");
	}
};
